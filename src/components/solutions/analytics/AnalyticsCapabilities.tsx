...
function NLQueryPanel() {
  ...
  return (
    <div className="mx-auto w-full max-w-[350px] rounded-3xl border border-indigo-100 bg-slate-50 p-4 sm:p-5 shadow-lg">
      <div className="rounded-2xl border border-indigo-50 bg-white p-4 shadow-sm space-y-3.5">
        {/* Query Input */}
        <div className="flex items-start gap-2.5 rounded-xl border border-indigo-200/70 bg-indigo-50/60 p-3">
        ...
        <div className="flex items-center gap-2">
          <span className="inline-flex items-center gap-1.5 rounded-full bg-violet-100 px-2.5 py-0.5 text-[11px] font-semibold text-violet-800">
            <FileSearch className="h-3 w-3" /> Consulta procesada
          </span>
        </div>
        ...
        <div>
          ...
          <div className="flex flex-col gap-1">
            {categories.map((cat) => (
              ...
              >
                <Circle className="h-1.5 w-1.5 text-indigo-400 fill-indigo-400 shrink-0" />
        ...
        <div className="rounded-xl border border-violet-100 bg-violet-50/60 p-2.5">
          <p className="text-[10px] font-bold text-violet-800 mb-1.5">Fuentes analizadas</p>
          <div className="flex flex-wrap gap-1">
            {sources.map((s) => (
              <span key={s} className="rounded-full border border-violet-200 bg-white px-2 py-0.5 text-[11px] text-violet-700 font-medium">
                {s}
              </span>
            ))}
          </div>
    ...
    <div className="mx-auto w-full max-w-[350px] rounded-3xl border border-indigo-100 bg-slate-50 p-4 sm:p-5 shadow-lg">
      <div className="rounded-2xl border border-indigo-50 bg-white p-4 shadow-sm space-y-3.5">
        ...
        <div className="flex justify-center">
          <div className="rounded-2xl border-2 border-indigo-400 bg-indigo-50 px-4 py-2 text-center shadow-sm">
            <p className="text-[10px] font-bold text-indigo-500 uppercase tracking-wide">Nodo central</p>
            <p className="text-xs font-bold text-indigo-900 mt-0.5">Actividad analizada</p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-1.5 justify-center">
          {clusters.map((c) => (
            <span
              key={c.label}
              className={`rounded-lg border px-2.5 py-1 text-[11px] font-semibold ${c.color}`}
            >
              {c.label}
            </span>
          ))}
        ...
        <div className="rounded-xl border border-amber-200/60 bg-amber-50/60 p-3">
          <p className="text-[10px] font-bold text-amber-800 mb-1">Hallazgo para revisar</p>
